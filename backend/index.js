...
// WHY NOT AN API KEY: any secret placed in frontend JavaScript is visible to
// anyone via browser DevTools -> Network tab, regardless of CORS settings.
// An API key sent from the browser provides zero real protection once deployed
// — it just becomes a public string. Browsers automatically send the Origin
// header on cross-origin requests and it cannot be forged by a normal page,
// so checking it server-side blocks direct curl/script access from outside
// your own deployed frontend without exposing any secret to the client.
//
// This is NOT equivalent to real authentication (a determined attacker can
// still spoof an Origin header with curl), but it stops casual scraping and
// quota abuse — which is the actual threat model here, since there is no
// per-user data to protect, only a shared API quota.
...
// ─── File Upload N/A — text IOC input only ───────────────────────────────────
...
async function queryAbuseIPDB(ip) {
  ...
  return {
    ...
    abuse_confidence: d.abuseConfidenceScore, total_reports: d.totalReports,
    distinct_reporters: d.numDistinctUsers, country: d.countryCode, isp: d.isp,
    domain: d.domain, is_tor: d.isTor, is_public: d.isPublic, usage_type: d.usageType,
...
async function queryURLhaus(ioc, type) {
  ...
  return {
    ...
    status: d.urlhaus_status || d.query_status, threat: d.threat || null,
    url_count: d.urls_count || null, blacklists: d.blacklists || null,
    urls: d.urls?.slice(0, 3).map(u => ({ url: u.url, status: u.url_status, threat: u.threat, date_added: u.date_added })) || []
  };
}
...
async function queryIPInfo(ip) {
  ...
  const d = result.data;
  return {
    source: 'IPInfo', available: true, ip: d.ip,
    city: d.city || null, region: d.region || null, country: d.country || null,
    org: d.org || null, asn: d.org?.split(' ')[0] || null,
    isp: d.org?.split(' ').slice(1).join(' ') || null,
    is_vpn: d.privacy?.vpn || false, is_tor: d.privacy?.tor || false,
    is_proxy: d.privacy?.proxy || false, is_hosting: d.privacy?.hosting || false,
    timezone: d.timezone || null, loc: d.loc || null
  };
}
...
async function queryAllSources(ioc, type) {
  const queries = [];
  if (type === 'ip') {
    queries.push(queryAbuseIPDB(ioc), queryOTX(ioc, 'ip'), queryVirusTotal(ioc, 'ip'), queryIPInfo(ioc), queryURLhaus(ioc, 'ip'));
  } else if (type === 'domain') {
    queries.push(queryOTX(ioc, 'domain'), queryVirusTotal(ioc, 'domain'), queryURLhaus(ioc, 'domain'));
  } else if (type === 'url') {
    queries.push(queryURLhaus(ioc, 'url'), queryVirusTotal(ioc, 'url'), queryOTX(ioc, 'url'));
  } else if (type === 'md5' || type === 'sha256') {
    queries.push(queryVirusTotal(ioc, type), queryOTX(ioc, type), queryURLhaus(ioc, type));
  }
...
${failedSources.length > 0 ? `UNAVAILABLE SOURCES: ${failedSources.map(s => s.source).join(', ')}` : ''}

ANALYSIS INSTRUCTIONS:
1. If sources disagree, explain WHY and which source is more authoritative for this IOC type
2. Weight sources by relevance: AbuseIPDB for IP reputation, VirusTotal for hashes, URLhaus for malicious URLs/domains
3. Be specific — reference actual numbers from the data
4. Only include MITRE ATT&CK techniques with DIRECT evidence in the data

Return ONLY a valid JSON object with exactly this structure:
{
  "threat_score": <integer 0-100>,
  "risk_level": "Clean" or "Suspicious" or "Malicious" or "Critical",
  "narrative": "<3-4 sentences referencing actual data points>",
  "mitre_techniques": [{"id": "T1090.003", "name": "Proxy: Multi-hop Proxy", "relevance": "one sentence"}],
  "recommendation": "<one specific immediately actionable step>",
  "confidence": <integer 0-100>,
  "threat_categories": ["<category1>", "<category2>"]
}

Scoring guide: 0-20 Clean, 21-40 Low risk, 41-60 Suspicious, 61-80 Malicious, 81-100 Critical`;
  ...
  const completion = await groq.chat.completions.create({
    ...
    messages: [
      { role: 'system', content: 'You are a senior threat intelligence analyst. You synthesize data from multiple threat intelligence sources into clear, actionable security assessments. You always return valid JSON only.' },
...
function extractGeo(sourceResults) {
  ...
  return {
    ...
    city: ipinfo?.city || null, region: ipinfo?.region || null,
    asn: ipinfo?.asn || otx?.asn || null, isp: ipinfo?.isp || abuse?.isp || null,
    is_tor: ipinfo?.is_tor || abuse?.is_tor || false, is_vpn: ipinfo?.is_vpn || false,
    is_proxy: ipinfo?.is_proxy || false, is_hosting: ipinfo?.is_hosting || false,
    loc: ipinfo?.loc || null
  };
}
...
app.get('/health', (req, res) => {
  const keys = {
    ...
    otx: !!process.env.OTX_KEY,
    virustotal: !!process.env.VT_KEY,
    ipinfo: !!process.env.IPINFO_KEY,
    groq: !!process.env.GROQ_KEY
  };
  res.json({ status: 'ok', service: 'ThreatLens API', keys_configured: keys, allowed_origins: ALLOWED_ORIGINS });
});
...
// originCheck applied ONLY to /api/analyze — health and test-api stay open for debugging
app.post('/api/analyze', originCheck, async (req, res) => {
  const { ioc } = req.body;
  ...
  if (!iocType) {
    return res.status(400).json({
      ...
      code: 'UNKNOWN_IOC_TYPE', input: cleaned
    });
  }
  ...
  try {
    ...
    const response = {
      ...
      sources_checked: sourceResults.map(s => ({ source: s.source, available: s.available, error: s.available ? undefined : s.error })),
      sources_detail: sourceResults,
      meta: { query_time_ms: queryTime, sources_queried: sourceResults.length, sources_available: availableCount, analyzed_at: new Date().toISOString() }
    };
    ...
    if (err.message?.includes('401') || err.message?.includes('invalid_api_key')) {
      return res.status(500).json({ error: 'Invalid Groq API key.', code: 'INVALID_GROQ_KEY' });
    }
    return res.status(500).json({ error: 'Analysis failed. Please try again.', code: 'SERVER_ERROR', ...(IS_DEV && { debug: err.message }) });
  }
});
...
app.listen(PORT, () => {
  const keys = {
    ABUSEIPDB: !!process.env.ABUSEIPDB_KEY, OTX: !!process.env.OTX_KEY,
    VIRUSTOTAL: !!process.env.VT_KEY, IPINFO: !!process.env.IPINFO_KEY, GROQ: !!process.env.GROQ_KEY
  };
  const allOk = Object.values(keys).every(Boolean);
  const keyStatus = Object.entries(keys).map(([k, v]) => `  ${v ? '[ok]' : '[missing]'} ${k}`).join('\n');
...
${keyStatus}
  ...
  ${allOk ? 'All keys configured — ready.' : 'Some keys missing — check .env file'}
